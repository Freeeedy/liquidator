...
Liquidator is a lightweight Python (Linux-only) tool to **sanitize files by removing metadata, resetting timestamps, and re-encoding content for privacy purposes**. It supports documents, images, videos, audio, eBooks, and design files using tools like `mat2`, `exiftool`, `ffmpeg`, `libreoffice`, and `ghostscript`.
...
- Requires: mat2, exiftool, ffmpeg, libreoffice, ghostscript
- Linux only
