...
if os_name == "Linux":
    ...
    if ext in doc_extensions + image_extensions + design_extensions:
        subprocess.run(f'exiftool -all= -overwrite_original "{file}"', shell=True)
        subprocess.run(f'powershell -Command "(Get-Item \'{file}\').LastWriteTime = \'2000-01-01T00:00:00\'"', shell=True)

    elif ext in text_extensions:
        subprocess.run(f'copy "{file}" newfile.{ext}', shell=True)
        subprocess.run(f'move "newfile.{ext}" "{file}"', shell=True)
        subprocess.run(f'powershell -Command "(Get-Item \\"{file}\\").LastWriteTime = (Get-Date \\"2000-01-01T00:00:00\\")"', shell=True)

    elif ext in video_extensions + audio_extensions:
        subprocess.run(f'ffmpeg -i "{file}" -map_metadata -1 -c:v copy -c:a copy newfile.{ext}', shell=True)
        subprocess.run(f'move newfile.{ext} -Destination "{file}"', shell=True)
        subprocess.run(f'exiftool -all= -overwrite_original "{file}"', shell=True)
        subprocess.run(f'powershell -Command "(Get-Item \'{file}\').LastWriteTime = \'2000-01-01T00:00:00\'"', shell=True)
